...
class Visual extends Component {
    ...
    async getVisualData(type = undefined) {
        ...
        for (let key in this.state.keyList) {
            let ep =
                `http://localhost:4000/api/cloud/data?key_id=${key}` +
                (type ? `&type=${type}` : "");
                        ...
                        for (let tmp_rds_group of rds_group) {
                            ...
                            if (tmp_vpc.id == tmp_rds_group.link[0]) {
                                for (let tmp_rds of rds) {
                                    for (
                                        var i = 0;
                                        i < tmp_rds.link.length;
                                        i++
                                    ) {
                                        if (
                                            tmp_rds.link[i] == tmp_rds_group.id
                                        ) {
                                            tmp_rds_group.children.push(
                                                tmp_rds
                                            );
                                        }
                                    }
            ...
            function update(stateFunc, preState) {
                ...
                nodeEnter
                    ...
                    .attr("xlink:href", function (d) {
                        if (d.data.type == "ec2") return "/images/compute.svg";
                        ...
                        if (d.data.type == "ebs") return "/images/ebs.svg";
                    })
                    .attr("x", function (d) {
                        return -30;
                    })
